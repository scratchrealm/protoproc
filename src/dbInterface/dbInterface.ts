import { CreateProjectRequest, CreateJobRequest, CreateWorkspaceRequest, DeleteFileRequest, DeleteProjectRequest, DeleteComputeResourceRequest, DeleteJobRequest, DeleteWorkspaceRequest, GetProjectsRequest, GetFileRequest, GetFilesRequest, GetProjectRequest, GetComputeResourcesRequest, GetDataBlobRequest, GetJobRequest, GetJobsRequest, GetWorkspaceRequest, GetWorkspacesRequest, RegisterComputeResourceRequest, SetFileRequest, SetProjectPropertyRequest, SetWorkspacePropertyRequest, SetWorkspaceUsersRequest, DuplicateFileRequest, RenameFileRequest, GetComputeResourceRequest, GetComputeResourceSpecRequest } from "../types/ProtoprocRequest";
import { ProtoprocProject, ProtoprocFile, ProtoprocComputeResource, ProtoprocJob, ProtoprocWorkspace } from "../types/protoproc-types";
import postProtoprocRequest from "./postProtoprocRequest";

export const fetchWorkspaces = async (auth: Auth): Promise<ProtoprocWorkspace[]> => {
    const req: GetWorkspacesRequest = {
        type: 'getWorkspaces',
        timestamp: Date.now() / 1000
    }
    const resp = await postProtoprocRequest(req, {...auth})
...
export const fetchWorkspace = async (workspaceId: string, auth: Auth): Promise<ProtoprocWorkspace | undefined> => {
...
export const fetchProjects = async (workspaceId: string, auth: Auth): Promise<ProtoprocProject[]> => {
...
export const fetchProject = async (projectId: string, auth: Auth): Promise<ProtoprocProject | undefined> => {
...
export const fetchFiles = async (projectId: string, auth: Auth): Promise<ProtoprocFile[]> => {
...
export const fetchFile = async (projectId: string, fileName: string, auth: Auth): Promise<ProtoprocFile | undefined> => {
...
export const fetchFileText = async (file: ProtoprocFile, auth: Auth): Promise<string | undefined> => {
...
export const fetchComputeResources = async (auth: Auth): Promise<ProtoprocComputeResource[]> => {
...
export const fetchComputeResource = async (computeResourceId: string, auth: Auth): Promise<ProtoprocComputeResource | undefined> => {
...
export type ProtoprocProcessingJobDefinition = {
    toolName: string,
    inputFiles: {
        name: string
        fileName: string
...
export const createJob = async (
    workspaceId: string,
    projectId: string,
    jobDef: ProtoprocProcessingJobDefinition,
    auth: Auth)
: Promise<string> => {
...
export const fetchJobsForProject = async (projectId: string, auth: Auth): Promise<ProtoprocJob[]> => {
...
export const fetchJobsForComputeResource = async (computeResourceId: string, auth: Auth): Promise<ProtoprocJob[]> => {
    const req: GetJobsRequest = {
        type: 'getJobs',
        timestamp: Date.now() / 1000,
        computeResourceId
...
export const fetchJob = async (jobId: string, auth: Auth): Promise<ProtoprocJob | undefined> => {
...
export const getComputeResourceSpec = async (computeResourceId: string): Promise<any> => {
    ...
    const resp = await postProtoprocRequest(req, {})
