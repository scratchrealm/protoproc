...
import { createJob, ProtoprocProcessingJobDefinition } from "../../../../dbInterface/dbInterface"
...
const RunSpikeSortingWindow: FunctionComponent<RunSpikeSortingWindowProps> = ({fileName, onClose, spikeSortingToolName, nwbFile}) => {
    ...
    const [jobDefinition, setJobDefinition] = useState<ProtoprocProcessingJobDefinition | undefined>(undefined)
    useEffect(() => {
        if (!spikeSortingTool) return
        const ss = new ProcessingToolSchemaParser(spikeSortingTool.schema)
        const jd: ProtoprocProcessingJobDefinition = {
            inputFiles: [
                {
                    name: 'input',
                    fileName
