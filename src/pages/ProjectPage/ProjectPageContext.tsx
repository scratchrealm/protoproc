...
import { ProtoprocFile, ProtoprocJob, ProtoprocProject } from '../../types/protoproc-types';
...
export const SetupProjectPage: FunctionComponent<PropsWithChildren<Props>> = ({children, projectId}) => {
    const [project, setProject] = React.useState<ProtoprocProject | undefined>()
    const [files, setFiles] = React.useState<ProtoprocFile[] | undefined>()
    const [refreshFilesCode, setRefreshFilesCode] = React.useState(0)
    const refreshFiles = useCallback(() => setRefreshFilesCode(rfc => rfc + 1), [])

    const [jobs, setJobs] = React.useState<ProtoprocJob[] | undefined>(undefined)
    ...
    const [previousJobs, setPreviousJobs] = React.useState<ProtoprocJob[] | undefined>(undefined)
    useEffect(() => {
        if (!jobs) return
        if (previousJobs) {
