import { RegisterComputeResourceRequest, RegisterComputeResourceResponse } from "../../src/types/ProtoprocRequest";
import { ProtoprocComputeResource } from "../../src/types/protoproc-types";
...
const registerComputeResourceHandler = async (request: RegisterComputeResourceRequest, o: {verifiedClientId?: string, verifiedUserId?: string}): Promise<RegisterComputeResourceResponse> => {
    ...
    if (cr) {
        ...
        const x: ProtoprocComputeResource = {
            computeResourceId,
            ownerId: userId,
            name: request.name,
            timestampCreated: Date.now() / 1000
