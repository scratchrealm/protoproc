import { DeleteFileRequest, DeleteFileResponse } from "../../src/types/ProtoprocRequest";
import getProject from '../getProject';
import { getMongoClient } from "../getMongoClient";
import getWorkspace from '../getWorkspace';
import { userCanDeleteFile } from "../permissions";
import removeIdField from "../removeIdField";
import { isProtoprocDataBlob, isProtoprocFile, isProtoprocJob, ProtoprocDataBlob, ProtoprocFile, ProtoprocJob } from "../../src/types/protoproc-types";
...
export const cleanupProject = async (projectId: string) => {
    ...
    let files = removeIdField(await filesCollection.find({projectId}).toArray()) as ProtoprocFile[]
    files.forEach(file => {
    ...
    let jobs = removeIdField(await jobsCollection.find({projectId}).toArray()) as ProtoprocJob[]
    jobs.forEach(job => {
        if (!isProtoprocJob(job)) {
            console.warn(job)
            throw new Error('Invalid job in database')
        }
    })
    let dataBlobs = removeIdField(await dataBlobsCollection.find({projectId}).toArray()) as ProtoprocDataBlob[]
    dataBlobs.forEach(dataBlob => {
        if (!isProtoprocDataBlob(dataBlob)) {
