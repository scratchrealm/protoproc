...
def run_job():
    from .ProtoprocPluginTypes import ProtoprocPlugin, ProtoprocPluginContext, ProtoprocProcessingTool, ProtoprocProcessingToolContext, InputFile, OutputFile
    ...
    processing_tools: list[ProtoprocProcessingTool] = []
    ...
    class ProtoprocProcessingToolContextImpl(ProtoprocProcessingToolContext):
        ...
        def upload_output_file(self, output_file: OutputFile, path: str):
            ...
            s3.upload_file(path, OUTPUT_BUCKET, f'protoproc-dev/{random_output_id}/{basename}')
            # for now we hard-code neurosift.org
            url = f'{OUTPUT_BUCKET_BASE_URL}/protoproc-dev/{random_output_id}/{basename}'
    ...
    tool.run(ProtoprocProcessingToolContextImpl())
