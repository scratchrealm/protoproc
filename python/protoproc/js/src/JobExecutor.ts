import fs from 'fs'
import postProtoprocRequestFromComputeResource from "./postProtoprocRequestFromComputeResource"
import PubsubClient from './PubsubClient'
import JobManager from "./JobManager"
import { GetPubsubSubscriptionRequest, GetJobsRequest, SetComputeResourceSpecRequest } from "./types/ProtoprocRequest"
...
class JobExecutor {
    ...
    constructor(private a: { dir: string }) {
        // read computeResourceId from .protoproc-compute-resource-node.yaml in dir directory
        ...
        if (container_method === 'none') {
            ...
            if (process.env.PROTOPROC_DANGEROUS_CONTAINER_METHOD_NONE !== 'true') {
                throw Error('Container method is set to none. Set environment variable PROTOPROC_DANGEROUS_CONTAINER_METHOD_NONE to true to allow this.')
            }
        }
    ...
    async start() {
        ...
        if (fs.existsSync(specJsonFname)) {
            ...
            const respSpec = await this._postProtoprocRequest(reqSpec)
            if (!respSpec) {
        ...
        const respPubsub = await this._postProtoprocRequest(reqPubsub)
    ...
    private async _processPendingJobs() {
        ...
        const resp = await this._postProtoprocRequest(req)
        if (resp) {
    ...
    private async _postProtoprocRequest(req: any): Promise<any> {
        return await postProtoprocRequestFromComputeResource(req, {
            computeResourceId: process.env.COMPUTE_RESOURCE_ID,
