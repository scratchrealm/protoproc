...
def handle_job(*, job_id: str):
    config_fname = '.protoproc-compute-resource-node.yaml'
    ...
    try:
        ...
        for a in job['inputFiles']:
            ...
            resp = _post_protoproc_request(req)
        ...
        # run "protoproc run-job" in the job directory
...
def _post_protoproc_request(req):
    ...
    protoproc_url = os.environ.get('PROTOPROC_URL', 'https://protoproc.vercel.app')
    resp = requests.post(f'{protoproc_url}/api/protoproc', json=rr)
    if resp.status_code != 200:
        msg = resp.text
        raise ValueError(f'Error posting protoproc request: {msg}')
    return resp.json()